...
/*
	...
	first of all, let's count how many digits are within "nb", that's what
	the function "len_num(long int nb)" does as it checks if "nb"
	is negative to consider the sign '-' or "0" as characters otherwise
	it counts how many digits by dividing the "nb" by 10 and
	returns the result
	...
	ft_itoa creates a variable "nb" to allocate "n" in enough memory
	then allocates the proper size based on the result that comes from 
	"len_num(int n)" and if the allocations fail "NULL" will be returned.

	the function returns a string so putting "NULL Character" is necessary to
	indicate the end of the string that's why we put it in "p[i]" since "i" holds
	...
	positive integer to operate for converting each digit into 
